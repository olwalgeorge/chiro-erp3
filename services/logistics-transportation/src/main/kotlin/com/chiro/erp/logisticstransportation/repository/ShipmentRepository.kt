...
class ShipmentRepository : PanacheRepository<Shipment> {
    ...
    fun findByCustomerId(customerId: String): Uni<List<Shipment>> =
        find("customerId", customerId).list()

    fun findByStatus(status: ShipmentStatus): Uni<List<Shipment>> =
        find("status", status).list()

    fun findByOrigin(origin: String): Uni<List<Shipment>> =
        find("origin", origin).list()

    fun findByDestination(destination: String): Uni<List<Shipment>> =
        find("destination", destination).list()
    ...
    fun findActiveShipments(): Uni<List<Shipment>> =
        find("status IN (?1, ?2, ?3, ?4)", 
            ...
            ShipmentStatus.OUT_FOR_DELIVERY).list()

    fun findDelayedShipments(): Uni<List<Shipment>> =
        find("status = ?1", ShipmentStatus.DELAYED).list()
}
