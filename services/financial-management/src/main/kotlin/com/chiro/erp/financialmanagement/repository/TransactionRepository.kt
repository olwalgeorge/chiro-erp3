...
class TransactionRepository : PanacheRepository<Transaction> {
    ...
    fun findByAccountId(accountId: String): Uni<List<Transaction>> =
        find("accountId", accountId).list()

    fun findByStatus(status: TransactionStatus): Uni<List<Transaction>> =
        find("status", status).list()

    fun findByType(type: TransactionType): Uni<List<Transaction>> =
        find("type", type).list()

    fun findByAccountIdAndType(accountId: String, type: TransactionType): Uni<List<Transaction>> =
        find("accountId = ?1 AND type = ?2", accountId, type).list()
